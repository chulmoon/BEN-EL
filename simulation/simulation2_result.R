...
medse.ben = matrix(NA,length(datnum),2)
select.ben = matrix(NA,length(datnum),2)

for (sim in 1:length(datnum)) {
  load(paste0("./simulation/sim_2_",datnum[sim],"_BEN.Rdata"))
  load(paste0("./simulation/data/sim_2_",datnum[sim],".Rdata"))
  
	p=ncol(result.ben[[1]]$x.train)
	...
	select.ben[sim,] = c(mean(apply(bsm.ben[,1:15],2,sum)),mean(apply(bsm.ben[,16:30],2,sum)))
...
# BL
#####################################################################
medse.bl = matrix(NA,length(datnum),2)
select.bl = matrix(NA,length(datnum),2)

for (sim in 1:length(datnum)) {
  load(paste0("./simulation/sim_2_",datnum[sim],"_BL.Rdata"))
  load(paste0("./simulation/data/sim_2_",datnum[sim],".Rdata"))
  
	p=ncol(result.bl[[1]]$x.train)
	...
	select.bl[sim,] = c(mean(apply(bsm.bl[,1:15],2,sum)),mean(apply(bsm.bl[,16:30],2,sum)))
...
# BENEL
#####################################################################
medse.benel = matrix(NA,length(datnum),2)
select.benel = matrix(NA,length(datnum),2)

for (sim in 1:length(datnum)) {
  load(paste0("./simulation/sim_2_",datnum[sim],"_BENEL.Rdata"))
  conv=NULL
  for (ii in 1:100){
...
for (sim in 1:length(datnum)) {
  ...
  load(paste0("./simulation/data/sim_2_",datnum[sim],".Rdata"))
	...
	select.benel[sim,] = c(mean(apply(bsm.benel[,1:15],2,sum)),mean(apply(bsm.benel[,16:30],2,sum)))
...
# EN
#####################################################################
medse.en = matrix(NA,length(datnum),2)

for (sim in 1:length(datnum)) {
  load(paste0("./simulation/sim_2_",datnum[sim],"_EN.Rdata"))
  median.bsample = NULL
	for (b in 1:1000) {
...
# LADL
#####################################################################
medse.ladlasso = matrix(NA,length(datnum),2)

for (sim in 1:length(datnum)) {
  load(paste0("./simulation/sim_2_",datnum[sim],"_LADL.Rdata"))
  median.bsample = NULL
  for (b in 1:1000) {
  ...
  print(c(mean(100-apply(bsm.ladlasso[,1:15],2,sum)), mean(100-apply(bsm.ladlasso[,16:30],2,sum))))
}
medse.ladlasso
